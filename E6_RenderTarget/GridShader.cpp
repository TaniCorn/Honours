...
GridShader::GridShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"grid_vs.cso", L"grid_ps.cso");
}
...
void GridShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void GridShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& world, const XMMATRIX& orthoView, const XMMATRIX& ortho, const XMMATRIX& view, const XMMATRIX& projection, ID3D11ShaderResourceView* texture, XMFLOAT3 camerapos)
{
	HRESULT result;
